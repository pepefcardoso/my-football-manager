...
export function NewGameModal({ onClose, onConfirm }: NewGameModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (invalidChars.test(saveName)) {
            setError("O nome do save contém caracteres inválidos. Use apenas letras, números, espaços, hífens ou underlines.");
            return;
        }
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md shadow-2xl p-6">
                ...
                <p className="text-slate-400 text-sm mb-6">Configure sua carreira antes de escolher o time.</p>
