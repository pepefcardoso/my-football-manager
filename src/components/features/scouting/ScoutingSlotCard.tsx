    ...
    `}>
            ...
            {isActive ? (
                <div className="space-y-3">
                    <div className="text-sm text-slate-200">
                        ...
                        <ul className="space-y-1 text-xs text-slate-400">
                            <li>🌍 {slot.filters.country || "Global"}</li>
                            <li>⚽ {slot.filters.position || "Qualquer Posição"}</li>
                            <li>🎂 {slot.filters.ageGroup ? `Idade: ${slot.filters.ageGroup}` : "Qualquer Idade"}</li>
                        </ul>
                    </div>
                    ...
                    <div className="pt-3 border-t border-slate-700 flex justify-between items-center">
                        <div className="text-xs">
                            ...
                            <span className="text-white font-mono font-bold text-lg">{slot.stats.playersFound}</span>
                        </div>
                        <button
                            onClick={() => onStop(slot.slotNumber)}
                ...
                <div className="flex flex-col items-center justify-center py-6 text-center">
                    <button
                        onClick={() => onConfigure(slot.slotNumber)}
